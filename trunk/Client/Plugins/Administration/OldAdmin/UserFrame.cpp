...
UserFrame::UserFrame() : QFrame()
{
    setupUi(this);
    fillUserFrame();
    requestFunctions["changePassword"]  = &UserFrame::changePasswordResponse;
    requestFunctions["listUsers"]       = &UserFrame::listUsersResponse;
    requestFunctions["getUserInfo"]     = &UserFrame::getUserInfoResponse;
    requestFunctions["setUserInfo"]     = &UserFrame::setUserInfoResponse;
    requestFunctions["createNewUser"]   = &UserFrame::createNewUserResponse;
    requestFunctions["disableUser"]     = &UserFrame::disableUserResponse;
    requestFunctions["getAllTree+"]         = &UserFrame::getTreeResponse;
    requestFunctions["getNodeInfo"]     = &UserFrame::getNodInfoResponse;
    requestFunctions["setNode"]         = &UserFrame::editNodeResponse;
...
void    UserFrame::on_AddButton_clicked()
{
    this->createNewUser();
}

void    UserFrame::on_SaveButton_clicked()
{
    QMessageBox msgBox;
...
void    UserFrame::on_DelButton_clicked()
{
    qDebug() << "Del";
}

void    UserFrame::on_CancelButton_clicked()
{
    QMessageBox msgBox;
...
void    UserFrame::changeEvent(QEvent *e)
{
    switch (e->type()) {
...
void UserFrame::itemClicked(QTreeWidgetItem *item, int idx)
{
    this->AddButton->hide();
...
void    UserFrame::setUserInfo()
{
    ...
    emit sender("students", pP);

}

void    UserFrame::createNewUser()
{
    QVariantHash request;
    QString      Error = "";
...
void    UserFrame::disableUser()
{
    QVariantHash request;

    request["Request"] =  "disableUser";
//    emit sender("students"); // How this function can work without any network request ???? (Valentin)
}

void    UserFrame::readResponse(QVariantHash response)
{

    (this->*requestFunctions.value(response["Request"].toByteArray(),
                                   &UserFrame::unknownResponse))
                                   (response);
}

void    UserFrame::changePasswordResponse(QVariantHash &response)
{
    qDebug() << response;
}

void    UserFrame::listUsersResponse(QVariantHash &response)
{
...
void    UserFrame::getUserInfoResponse(QVariantHash &response)
{
    qDebug() << response;
}

void    UserFrame::setUserInfoResponse(QVariantHash &response)
{
    QMessageBox msgBox;
    QString msg;
...
void    UserFrame::createNewUserResponse(QVariantHash &response)
{
    QMessageBox msgBox;
    QString msg;
    if (response["Success"] == true)
...
void    UserFrame::disableUserResponse(QVariantHash &response)
{
    qDebug() << response;
}


void    UserFrame::unknownResponse(QVariantHash &response)
{
    qDebug() << response;
}
void    UserFrame::updateTree(QVariantHash tree)
{
...
void    UserFrame::getTree()
{
    PluginPacket *pP = new PluginPacket("TreeManagement", "getAllTree+");
	emit sender("user", pP);
}

void    UserFrame::getNodInfo()
{
    QVariantHash request;
...
void            UserFrame::deleteNode(QString idNode)
{
    QVariantHash request;
...
void            UserFrame::moveNode(QString from, QString to)
{
    QVariantHash request;
...
void            UserFrame::editNode(QString id, QString type, QString name, QString userId)
{
    QVariantHash request;
...
void    UserFrame::getNodInfoResponse(QVariantHash &response)
{
    qDebug() << response;
}

void    UserFrame::editNodeResponse(QVariantHash &response)
{
